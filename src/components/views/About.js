...
function SectionTitle({ title, subtitle }) {
    return (
        <div style={{ margin: '3em 0 1.5em 0' }}>
            ...
            {subtitle && <p style={{ fontSize: '1.05em', color: 'var(--text-secondary)', margin: 0, fontWeight: 'lighter' }}>{subtitle}</p>}
        </div>
    );
}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {items.map((job, i) => (
                <div key={i} style={{ borderLeft: '3px solid var(--window-barrier)', paddingLeft: '1.5em', paddingBottom: i === items.length - 1 ? 0 : '3em', position: 'relative' }}>
                    ...
                    <p style={{ fontSize: '1.15em', color: 'var(--text-secondary)', margin: '0.2em 0 1em 0', fontWeight: 'lighter' }}>{job.company}</p>
                    {job.highlights && job.highlights.length > 0 && (
                        <ul style={{ padding: '0 0 0 1.2em', margin: 0, color: 'var(--text-secondary)', fontSize: '1em', lineHeight: '1.6', fontWeight: 'lighter' }}>
        ...
        <div className="skills-bento-grid">
            {categories.map((cat, i) => (
                <div key={i} style={{ background: 'var(--card-bg)', border: '1px solid var(--window-barrier)', borderRadius: '1em', padding: '1.5em', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <p style={{ fontSize: '0.9em', color: 'var(--text-secondary)', margin: '0 0 1.5em 0', flexGrow: 1, fontWeight: 'lighter' }}>{cat.description}</p>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5em' }}>
                        {cat.items.map((skill, j) => (
                            <span key={j} style={{ padding: '0.3em 0.8em', borderRadius: '2em', fontSize: '0.85em', background: 'var(--window-bg)', color: 'var(--text-secondary)', border: '1px solid var(--window-barrier)', fontWeight: 'lighter' }}>{skill}</span>
                        ))}
                    </div>
                </div>
            ))}
    ...
    return (
        <div className="view-container fade-in">
            ...
            <blockquote className="about-philosophy stagger-2" style={{ margin: '3em auto', padding: '0', maxWidth: '800px', textAlign: 'center' }}>
                ...
                <p style={{ color: 'var(--text-primary)', lineHeight: '1.8', fontSize: '1.4em', fontWeight: 'lighter', margin: '0 1em' }}>
